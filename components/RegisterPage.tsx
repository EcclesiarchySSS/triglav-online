...
const RegisterPage: React.FC = () => {
  ...
  const validateAge = (birthDate: string) => {
    ...
    return today.getFullYear() - birth.getFullYear() >= 15;
  };
  ...
  const validateForm = () => {
    ...
    else if (!validatePassword(formData.password)) newErrors.password = 'Пароль должен содержать минимум 8 символов';
    ...
    else if (!validateNickname(formData.nickname)) newErrors.nickname = 'Никнейм должен содержать 3-20 символов (буквы, цифры, _ -)';

    if (!formData.birthDate) newErrors.birthDate = 'Дата рождения обязательна';
    else if (!validateAge(formData.birthDate)) newErrors.birthDate = 'Вам должно быть не менее 15 лет';

    if (!formData.agreeToTerms) newErrors.agreeToTerms = 'Необходимо принять правила';
        ...
        <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">
              Создайте аккаунт и присоединяйтесь к битве
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black border border-amber-900/30 p-8 md:p-10 shadow-2xl">
            ...
            {errors.password && <p className="text-red-500 text-sm mt-[-10px] mb-4">{errors.password}</p>}
            {errors.confirmPassword && <p className="text-red-500 text-sm mt-[-10px] mb-4">{errors.confirmPassword}</p>}
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full bg-black/50 border ${errors.birthDate ? 'border-red-500' : 'border-amber-900/30'} pl-11 pr-4 py-3 text-white focus:outline-none focus:border-amber-600 transition scheme-dark`}
                />
              </div>
              {errors.birthDate && (
                <p className="text-red-500 text-sm mt-1 flex items-center">
            ...
            <div className="mb-8">
              <label className="flex items-start space-x-3 cursor-pointer group">
                ...
                <span className="text-sm text-gray-400 group-hover:text-gray-300 transition">
                  Я принимаю{' '}
                  <button type="button" className="text-amber-500 hover:text-amber-400 underline">
                    правила сайта
                  </button>
                </span>
              </label>
