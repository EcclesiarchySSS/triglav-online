...
const UserProfile: React.FC = () => {
  ...
  const handleCreateCharacter = () => {
    ...
    localStorage.setItem('triglav_user', JSON.stringify(updatedUser));
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button 
                ...
                className="flex items-center space-x-2 text-gray-400 hover:text-red-500 transition text-sm ml-4 uppercase font-bold"
              >
                <LogOut size={18} />
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-gray-900 border border-amber-900/30 p-6 rounded-sm">
              ...
              <p className="text-gray-500 text-center text-sm mb-4 uppercase font-bold tracking-wider">
                {user.role === 'admin' ? 'Администратор' : 'Рекрут'}
              </p>
              
              <div className="space-y-2">
                <button className="w-full flex items-center space-x-3 px-4 py-2 bg-amber-900/20 text-amber-500 border-l-2 border-amber-500 text-sm font-bold uppercase">
                  <Activity size={18} />
                  <span>Обзор</span>
                </button>
                <button className="w-full flex items-center space-x-3 px-4 py-2 text-gray-400 hover:bg-gray-800 hover:text-white transition text-sm font-bold uppercase">
                  <Settings size={18} />
                  <span>Настройки</span>
                </button>
                {user.role === 'admin' && (
                  <button onClick={() => navigate('/admin/dashboard')} className="w-full flex items-center space-x-3 px-4 py-2 text-red-400 hover:bg-gray-800 hover:text-red-300 transition text-sm font-bold uppercase">
                    <Shield size={18} />
                    <span>Админка</span>
                  </button>
                )}
            ...
            <div className="bg-gradient-to-br from-amber-900/20 to-black border border-amber-900/30 p-6 rounded-sm">
              ...
              <div className="flex items-center justify-between mb-2 border-b border-gray-800 pb-2">
                <span className="text-gray-400 text-sm">Средиземье</span>
                <span className={`${getStatusColor(serverStatus)} flex items-center text-sm font-bold uppercase`}>
                  <span className={`w-2 h-2 ${getStatusColor(serverStatus).replace('text-', 'bg-')} rounded-full mr-2`}></span>
                  {getStatusText(serverStatus)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400 text-sm">Арена</span>
                 <span className={`${getStatusColor(serverStatus)} flex items-center text-sm font-bold uppercase`}>
                  <span className={`w-2 h-2 ${getStatusColor(serverStatus).replace('text-', 'bg-')} rounded-full mr-2`}></span>
                  {getStatusText(serverStatus)}
                </span>
            ...
            <div className="bg-gray-900 border border-amber-900/30 p-6 rounded-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <button 
                  ...
                  className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-sm text-xs font-black uppercase tracking-widest flex items-center transition"
                >
              ...
              {(!user.characters || user.characters.length === 0) ? (
                 <div className="text-center py-12 text-gray-500 border-2 border-dashed border-gray-800 rounded-lg bg-black/20">
                    ...
                    <p className="text-lg font-bold">У вас пока нет персонажей.</p>
                    ...
                    <div key={char.id} className="bg-black/40 border border-gray-800 p-4 rounded flex items-center justify-between hover:border-amber-600 transition group cursor-pointer">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">Уровень {char.level} • {char.class}</p>
                        </div>
                      </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-gray-900 border border-amber-900/30 p-6 rounded-sm">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-white uppercase">Статус доступа</h3>
                    <p className="text-gray-500 text-sm">Ваша заявка на тестирование</p>
                  </div>
                  <Shield className="text-amber-600" size={24} />
                </div>
                <div className="inline-block px-3 py-1 bg-yellow-900/30 text-yellow-500 text-xs font-black uppercase rounded-sm border border-yellow-700/50">
                  На рассмотрении
                </div>
              ...
              <div className="bg-gray-900 border border-amber-900/30 p-6 rounded-sm">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-white uppercase">Игровой клиент</h3>
